...
For the local, SSH, and `herdr --remote` workflows, see [How to work with Herdr](./how-to-work/).
...
When Herdr starts again after a full server stop, it restores the saved session shape. For what survives detach, server restart, screen history replay, native agent session restore, and live handoff, see [Session state and restore](./session-state/).
